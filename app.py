...
# These are the "Pro" suggested defaults. 
# You can amend these via the UI and they will stay until you refresh.
FILTER_DEFAULTS = {
    "st_rvol": 1.5,
    "st_1m_trend": 0.0,
    "st_6m_trend": 0.0,
    "st_ath_dist": -20.0,  # Tightened from -35
    "st_max_rsi": 80.0,    # Loosened from 85 for momentum
    "st_max_bbpos": 1.1,
    "st_max_bw": 95.0,
    "st_max_accel": 5.0    # Loosened from 4.0
}
...
def calculate_metrics(ticker):
    try:
        ...
        # STRICT FILTERS (Dynamic Settings)
        if strict_mode:
            reason = None
            ...
            # NOTE: Earnings filter has been removed from here as requested.
...
def apply_rag_styles(df_full):
    df_disp = df_full.drop(columns=list(RAG_MAP.values()))
    style_df = pd.DataFrame("", index=df_disp.index, columns=df_disp.columns)
    for disp, num in RAG_MAP.items():
